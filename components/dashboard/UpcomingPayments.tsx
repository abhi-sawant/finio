...
function CreditPaymentRow({ account, onPayPress }: CreditPaymentRowProps) {
  ...
  return (
    <View style={styles.row}>
      {/* Left: icon + info */}
      <View
        style={[styles.iconCircle, { backgroundColor: hexToRgba(account.color, 0.2) }]}
      >
      ...
      {/* Pay button */}
      <TouchableOpacity
        style={[styles.payBtn, { backgroundColor: hexToRgba(account.color, 0.15), borderColor: hexToRgba(account.color, 0.4) }]}
        onPress={() => {
          lightHaptic()
          onPayPress(account)
        }}
